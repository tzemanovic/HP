...
namespace hp_fp
{
	...
	{
		EngineMut* engine = (EngineMut*) GetWindowLongPtr( handle, GWL_USERDATA );
		switch ( message )
		{
		case WM_CREATE:
		{
			CREATESTRUCT *cs = (CREATESTRUCT*) lParam;
			engine = (EngineMut*) cs->lpCreateParams;
			SetLastError( 0 );
			if ( SetWindowLongPtr( handle, GWL_USERDATA, (LONG_PTR) engine ) == 0 )
			{
				if ( GetLastError( ) != 0 )
				{
					return FALSE;
