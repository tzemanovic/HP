...
struct cohen_tag_descriptor
{
    size_t depth;
    size_t base_types[8];
};

cohen_tag_descriptor cohen[NUMBER_OF_DERIVED] = {};
...
struct shape_kind : shape_kind<N/2>
{
    ...
    shape_kind(size_t n = tag<N>::value) : base_class(n) { this->m_all_kinds = &cohen[N].base_types[0]; }
...
struct shape_kind<0> : OtherBase, Shape
{
    ...
    shape_kind(size_t n = tag<0>::value) : base_class(n) { this->m_all_kinds = &cohen[0].base_types[0]; }
    void accept(ShapeVisitor&) const;
};
...
template <size_t N> struct depth     { static const size_t value = requires_bits<N>::value+1; };
...
inline const shape_kind<N>* constant_time_dynamic_cast_ex(const shape_kind<N>*, const Shape* s)
{
    ...
    const size_t s_depth = s->m_all_kinds[-1];
    const size_t t_depth = depth<N>::value;
    return s_depth >= t_depth && s->m_all_kinds[t_depth-1] == t_tag
...
int main()
{
    ...
    for (int i = 0; i < NUMBER_OF_DERIVED; ++i)
    {
        ...
        for (int j = m-1, k=i; j>=0; --j,k = k/2)
        {
            size_t tag = get_tag(k);
            cohen[i].base_types[j] = tag;
            std::cout << tag << ',';
        }
