...
namespace hp_fp
{
	...
	struct GameInputMut
	{
		...
		GameInputMut( const GameInputMut& gi ) : states( gi.states ), mouse( gi.mouse ),
			text( gi.text ), deltaMs( gi.deltaMs ), timeMs( gi.timeMs )
		{ }
		GameInputMut( GameInputMut&& gi ) : states( std::move( gi.states ) ),
	...
	private:
		GameInputMut( std::array<bool, STATES_SIZE> states, MouseMut mouse, UInt32 text,
			double deltaMs, double timeMs )
			: states( states ), mouse( mouse ), text( text ), deltaMs( deltaMs ), timeMs( timeMs )
		{ }
